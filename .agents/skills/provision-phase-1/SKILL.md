...
- **`infra/data/shared/*` does NOT persist across `tofu destroy` as of 2026-07+** → that's by design. The `null_resource.wipe_data` destroy provisioner in `infra/tofu/cluster.tf` sweeps `infra/data/shared/` on every `tofu destroy` (paired with default `rm -rf` local-path teardown), and the kind `extra_mounts.propagation` is set to `Bidirectional` so the kernel sees container umounts as host-side events. To opt back into the 2026-06 "preserve across recreate" contract, pass `-var=preserve_stateful_data=true` to `tofu apply` AND run `bootstrap --destroy --preserve-data` (the two must agree) AND flip the local-path teardown script back to `mv` — see the comments in `infra/tofu/cluster.tf` and `infra/scripts/bootstrap/phase2/local_path_provisioner.py`.
