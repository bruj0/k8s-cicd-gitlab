  ...
  kind_config {
    ...
    dynamic "node" {
      ...
      content {
        ...
        # No extra_port_mappings: kind has no LoadBalancer, so the
        # Envoy Gateway data-plane Service (NodePort type) is not
        # reachable from the host. Access is via the bootstrap CLI's
        # --port-forward gitlab, which binds 127.0.0.1:8443 (https)
        # + 8222 (ssh) + 8080 (http-redirect) to the data-plane
        # ClusterIP. Keeping the kind control-plane clean of host-port
        # claims also leaves 80/443 free for system services.
